...
import { useResultOperation, useParallelResultOperations, useResultFormSubmission } from '../use-result-operation'
...
describe('useParallelResultOperations', () => {
  it('すべての操作が成功した場合、成功結果を返すこと', async () => {
    ...
    await act(async () => {
      await result.current.executeAll([mockFunction1, mockFunction2, mockFunction3], 'arg')
    })
  ...
  it('一つでも失敗した場合、失敗結果を返すこと', async () => {
    ...
    await act(async () => {
      await result.current.executeAll([mockFunction1, mockFunction2, mockFunction3])
    })
...
describe('useResultFormSubmission', () => {
  it('フォーム送信が成功した場合、適切に処理されること', async () => {
    ...
    const mockSubmitFunction = vi.fn().mockResolvedValue(success('form submitted'))
